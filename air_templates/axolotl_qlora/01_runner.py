...
# MAGIC must be an empty UC Volume directory.
...
# MAGIC checkpoints for resuming training. The separate merged directory and
# MAGIC Unity Catalog version contain full model weights with the adapter
# MAGIC applied. For remote gated models, edit the `HF_TOKEN` secret reference
# MAGIC in `train.yaml`; materialized local model/tokenizer paths do not require
