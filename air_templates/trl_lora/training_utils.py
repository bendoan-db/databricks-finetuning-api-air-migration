"""Distributed training, artifact, merge, and registration helpers for plain LoRA."""
...
def _load_model_and_tokenizer(
    config: dict[str, Any], model_reference: str, tokenizer_reference: str
) -> tuple[Any, Any]:
    """Load and configure the base model and tokenizer for plain LoRA."""
    import torch
    ...
    if getattr(model, "is_quantized", False):
        raise ValueError(
            "This is a plain LoRA template; quantized models are forbidden"
        )
    model.config.use_cache = not config["gradient_checkpointing"]
    return model, tokenizer


def _build_sft_config(config: dict[str, Any]) -> Any:
    """Build TRL supervised fine-tuning arguments for plain LoRA."""
...
def _log_training_contract(
    ...
    """Record the effective plain-LoRA training contract in MLflow."""
    import mlflow

    mlflow.set_tags(
        {
            "training_framework": "trl_sft_peft",
