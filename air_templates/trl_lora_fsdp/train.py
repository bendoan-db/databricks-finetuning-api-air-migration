"""Primary LoRA with FSDP full sharding training entry point for Databricks AI Runtime.
...
def run_training(
    ...
    """Run TRL plain-LoRA supervised fine-tuning with FSDP full sharding."""
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("TRL LoRA FSDP training requires CUDA")
    ...
    trainer = SFTTrainer(
        # SFTTrainer loads after SFTConfig initializes distributed state and
        # rank-0-efficient FSDP loading, then injects the LoRA adapter.
    ...
    if getattr(trainer.model, "is_quantized", False):
        raise ValueError("Plain LoRA FSDP training forbids quantized models")
    ...
    if not 0 < trainable_parameters < total_parameters:
        raise RuntimeError(
            "LoRA must train a non-zero strict subset of parameters; got "
    ...
    try:
        ...
        # save_model; Trainer restricts adapter filesystem writes to rank 0.
        ...
        if rank == 0:
            ...
            if not (adapter_dir / "adapter_config.json").is_file():
                raise FileNotFoundError("SFTTrainer did not save adapter_config.json")
            if not any(
                (adapter_dir / name).is_file()
                for name in ("adapter_model.safetensors", "adapter_model.bin")
            ):
                raise FileNotFoundError("SFTTrainer did not save LoRA adapter weights")
            mlflow.log_param("adapter_output_dir", config["output_dir"])
