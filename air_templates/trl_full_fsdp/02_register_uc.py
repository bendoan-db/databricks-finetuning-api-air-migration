...
# ruff: noqa: E402,F821
# MAGIC %md
# MAGIC # Register the TRL full-weight FSDP model in Unity Catalog
# MAGIC
# MAGIC Run this notebook after `01_runner` has produced the gathered full model
# MAGIC checkpoint. Enter the training MLflow run ID in the widget. The final
# MAGIC checkpoint path and Unity Catalog model target are read from `train.yaml`.
...
print(f"Full-model checkpoint: {training_config['output_dir']}")
print(f"UC model target: {training_config['registered_model_name']}")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Register the full model
# MAGIC
# MAGIC The caller needs `USE CATALOG`, `USE SCHEMA`, and permission to create
# MAGIC or update the configured registered model. This logs only the portable
# MAGIC model files to the selected MLflow run and creates a new UC model version;
# MAGIC Trainer checkpoints and optimizer state are excluded.
