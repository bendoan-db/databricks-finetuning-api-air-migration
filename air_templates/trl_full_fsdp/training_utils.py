"""Distributed training, artifact, merge, and registration helpers for full-weight FSDP."""
...
def _load_tokenizer(config: dict[str, Any], tokenizer_reference: str) -> Any:
    """Load and configure the tokenizer used by full-model FSDP training."""
...
def _build_sft_config(config: dict[str, Any], model_reference: str) -> Any:
    """Build TRL supervised fine-tuning arguments for full-model FSDP."""
    ...
    # its Accelerator so all checkpoint and final saves gather full state.
...
def _log_training_contract(
    ...
    """Record the effective full-model FSDP training contract in MLflow."""
    import mlflow

    mlflow.set_tags(
        {
            "training_framework": "trl_sft_fsdp",
...
def register_trained_model(
    ...
    """Log the gathered full checkpoint and register it in Unity Catalog."""
    ...
    # Exclude Trainer checkpoint directories, optimizer state, and scheduler
    # state. MLflow follows these temporary links and copies only portable
    # Transformers files from the final rank-0 full-state save.
