"""Primary plain LoRA training entry point for Databricks AI Runtime.
...
def run_training(
    ...
    """Run distributed TRL supervised fine-tuning for a plain LoRA adapter."""
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("TRL LoRA training requires CUDA")
    ...
    model, tokenizer = _load_model_and_tokenizer(
        config, model_reference, tokenizer_reference
    )
    trainer = SFTTrainer(
        model=model,
        args=_build_sft_config(config),
